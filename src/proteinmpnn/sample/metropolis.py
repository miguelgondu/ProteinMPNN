...
def sample_all(
    ...
    new_rev_sequenceT = (
        rev_sequence[:rev_position] + "A" + rev_sequence[rev_position + 1 :]
    )
    new_rev_sequenceC = (
        rev_sequence[:rev_position] + "G" + rev_sequence[rev_position + 1 :]
    )
    new_rev_sequenceG = (
        rev_sequence[:rev_position] + "C" + rev_sequence[rev_position + 1 :]
    )

    # convert from bp index to codon index and scores the new sequences
    fwd_idx = position // 3
    rev_idx = (rev_position) // 3
    new_score_array1A, new_score_array2A = score(
        new_fwd_sequenceA,
        new_rev_sequenceA,
        probs1,
        probs2,
    ...
    new_score_array1T, new_score_array2T = score(
        new_fwd_sequenceT,
        new_rev_sequenceT,
        probs1,
        probs2,
    ...
    new_score_array1C, new_score_array2C = score(
        new_fwd_sequenceC,
        new_rev_sequenceC,
        probs1,
        probs2,
    ...
    new_score_array1G, new_score_array2G = score(
        new_fwd_sequenceG,
        new_rev_sequenceG,
        probs1,
        probs2,
...
def score(
    ...
    try:
        ...
        sa1[fwd_idx] = (
            float("inf")
            if aa1 == "Z"
            else -torch.log(probs1[fwd_idx, amino_acid_position[aa1]])
        )
    except (KeyError, IndexError):
        pass
    ...
    try:
        ...
        sa2[rev_idx] = (
            float("inf")
            if aa2 == "Z"
            else -torch.log(probs2[rev_idx, amino_acid_position[aa2]])
        )
    except (KeyError, IndexError):
        pass
...
nucleic_acid_complement = {"A": "T", "T": "A", "C": "G", "G": "C"}
...
def na_sample(probs1, probs2):
    ...
    (
        ...
        metropolis,
        overhang_residues,
    ) = run_metropolis_from_flags(metro_file)
    ...
    num_codons = probs1.shape[0]
    num_nas = (
        num_codons * 3 + shift + overhang_residues * 3
    )  # +1 for single frame shift, +2 for double frame shift, +3 per overhang base
    ...
    while not no_z:
        for i in range(num_mutations):
            ...
            if combined_scores[random_min] < score_overall:
                ...
                fwd_sequence, rev_sequence = (
                    sequence_list1[random_min],
                    sequence_list2[random_min],
                )
                score1, score2 = scores1[random_min], scores2[random_min]
                score_overall = new_score
                score_array1, score_array2 = (
                    score_arrays1[random_min],
                    score_arrays2[random_min],
                )
                best_iter = i
        ...
        final_AAs1, final_AAs2 = BPs_to_AAs(
            fwd_sequence, rev_sequence, num_codons, shift
        )
...
def run_metropolis_from_flags(flags_file):
    ...
    gradient_start = args.get(
        "gradient_start", 0.5
    )  # Accepts ~55 percent of the sampled data
