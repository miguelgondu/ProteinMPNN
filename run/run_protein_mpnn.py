...
def run_protein_mpnn(args):
    ...
    if os.path.isfile(args.design_specs_json):
        ...
        raise ValueError(
            f"The design specs json was not found: {args.design_specs_json}"
        )
    ...
    with torch.no_grad():
        for ix, protein in enumerate(pdb_ds):
            ...
            (
                ...
                design_specs_dict, protein, experimental=args.experimental
            )
            (
                X,
                S,
            ...
            native_score, native_scores_per_res = (
            ...
            if "chain_key" in design_specs_dict:
                split_ali = [
                    os.path.join(output_folder, fi + ".fasta")
                    for fi in design_specs_dict["chain_key"].keys()
                ]
                split_af2 = [
                    os.path.join(output_folder, fi + ".csv")
                    for fi in design_specs_dict["chain_key"].keys()
                ]
            ...
            with open(ali_file, "w") as f:
                for temp in temperatures:
                    for j in range(NUM_BATCHES):
                        ...
                        if args.pairwise:
                            sample_dict, log_probs, true_chain_mask = (
                                model.pairwise_sample(
                                    X,
                                    randn_2,
                                    S,
                                    chain_M,
                                    ...
                                    invert_probs=args.destabilize,
                                )
                            )
                            scores, scores_per_res = (
                                sample_dict["score"],
                                sample_dict["score_per_res"],
                            )
                            ...
                            if tied_positions_dict == None:
                                ...
                                sample_dict = model.tied_sample(
                                    ...
                                    bidir_table_dir=model_weight_dir,
                                )
                            S_sample = sample_dict["S"]
                                    ...
                                    for fsplit, af2split in zip(split_ali, split_af2):
                                        with open(fsplit, "w") as fsp:
                                            fbase = os.path.splitext(
                                                os.path.basename(fsplit)
                                            )[0]
                                            chains_original = list(
                                                design_specs_dict["chain_key"][
                                                    fbase
                                                ].keys()
                                            )
                                            chains_to_recover = list(
                                                design_specs_dict["chain_key"][
                                                    fbase
                                                ].values()
                                            )
                                            chain_idx = [
                                                masked_list_list[0].index(ctr)
                                                for ctr in chains_to_recover
                                            ]
                                            native_seq_split = "/".join(
                                                [
                                                    native_seq.split("/")[idx]
                                            ...
                                            state_idx = torch.zeros(
                                                chain_encoding_all.shape,
                                                dtype=torch.bool,
                                                device=device,
                                            )

                                            for ctr, idx in zip(
                                                chains_original, chain_idx
                                            ):
                                                # grab the indices for scoring each chain
                                                score_idx = (
                                                    chain_encoding_all == idx + 1
                                                )
                                                chain_score = torch.sum(
                                                    native_scores_per_res[score_idx],
                                                    dim=-1,
                                                ) / torch.sum(
                                                    native_mask_for_loss[score_idx],
                                                    dim=-1,
                                                )
                                                chain_score = (
                                            ...
                                            score_per_state = torch.sum(
                                                native_scores_per_res[state_idx], dim=-1
                                            ) / torch.sum(
                                                native_mask_for_loss[state_idx], dim=-1
                                            )
                                            score_per_state = (
                                                np.format_float_positional(
                                                    np.float32(
                            ...
                            if "chain_key" in design_specs_dict:
                                ...
                                for fsplit, af2split in zip(split_ali, split_af2):
                                    with open(fsplit, "a") as fsp:
                                        ...
                                        chains_original = list(
                                            design_specs_dict["chain_key"][fbase].keys()
                                        )
                                        chains_to_recover = list(
                                            design_specs_dict["chain_key"][
                                                fbase
                                        ...
                                        chain_idx = [
                                            masked_list.index(ctr)
                                            for ctr in chains_to_recover
                                        ]
                                        seq_split = "/".join(
                                            [seq.split("/")[idx] for idx in chain_idx]
                                        )
                                        ...
                                        for ctr, idx in zip(chains_original, chain_idx):
                                            ...
                                            chain_score = torch.sum(
                                                scores_per_res[score_idx], dim=-1
                                            ) / torch.sum(
                                                mask_for_loss[score_idx], dim=-1
                                            )
                                            chain_score = np.format_float_positional(
                                                np.float32(
                                                    chain_score.cpu()
                                                    .data.numpy()
                                                    .mean()
                                            ...
                                            chain_seq_rec = torch.sum(
                                                seq_recovery_per_res.unsqueeze(0)[
                                                    score_idx
                                                ],
                                                dim=-1,
                                            ) / torch.sum(
                                        ...
                                        state_idx = state_idx.bool()
                                        score_per_state = torch.sum(
                                            scores_per_res[state_idx], dim=-1
                                        ) / torch.sum(mask_for_loss[state_idx], dim=-1)
                                        score_per_state = np.format_float_positional(
                                            np.float32(
                                                score_per_state.cpu()
                                                .data.numpy()
                                                .mean()
                                        ...
                                        seq_recovery_per_state = torch.sum(
                                            seq_recovery_per_res.unsqueeze(0)[
                                                state_idx
                                            ],
                                            dim=-1,
...
def run_protein_mpnn_func(
    ...
    if not os.path.isfile(ckpt_path):
        raise ValueError(
            "Invalid model name provided. Choose ca_48_002, ca_48_020, v_48_002, "
    ...
    with torch.no_grad():
        ...
        for ix, protein in enumerate(pdb_ds):
            ...
            (
                ...
                bias_AA_dict_decoy,
                tied_positions_dict,
                bias_by_res_dict_decoy,
            ) = transform_inputs(design_specs_dict, protein)
            print("=" * 50)
            print(
                chain_id_dict,
                "\n",
...
if __name__ == "__main__":
    ...
    parser = FileArgumentParser(
        description="Script that runs ProteinMPNN.", fromfile_prefix_chars="@"
    )
    ...
    parser.add_argument(
        "--destabilize",
        action="store_true",
        help="Include to invert aa probabilities by making less favored amino acids more common.",
    )
    ...
    parser.add_argument(
        "--pdb_dir", type=str, default="", help="Path to a single PDB to be designed"
    )
    parser.add_argument(
        "--design_specs_json",
        type=str,
    ...
    parser.add_argument(
        "--af2_formatted_output",
        action="store_true",
        help="Whether or not to include another output file that is in AF2 format for direct structure prediction after design.",
    )
    parser.add_argument(
        "--global_omit_AAs",
        type=str,
        default="X",
        help="AAs to globally omit from all designable positions (e.g. PC for no proline or cysteine). Note that it is generally advisable to include X in this list. Default is X.",
    )
    parser.add_argument(
        "--experimental",
        action="store_true",
        help="Enables experimental parsing of allowable mutations",
    )
    parser.add_argument(
        "--bidirectional",
        action="store_true",
        help="Enable bidirectional coding constraints. Default is off.",
    )
    parser.add_argument(
        "--bias_AA_dict",
        default=None,
        type=str,
        help="Path to json file containing global amino acid bias dictionary for MPNN. Default is None.",
    )
    parser.add_argument(
        "--bias_by_res_dict",
        default=None,
    ...
    parser.add_argument(
        "--pairwise",
        action="store_true",
        help="Enables parsing for experimental pairwise mutation clusters (experimental).",
    )
    parser.add_argument(
        "--mcmc",
        action="store_true",
        help="If enabled, bidirectional coding uses MCMC routine. Must have --bidirectional flag enabled.",
    )
    parser.add_argument(
        "--dump_probs",
        action="store_true",
        help="If enabled, predicted sequence probability tables (L x 20) will be dumped for each scaffold. If >1 sequence per scaffold is generated, probs are averaged before saving.",
    )
